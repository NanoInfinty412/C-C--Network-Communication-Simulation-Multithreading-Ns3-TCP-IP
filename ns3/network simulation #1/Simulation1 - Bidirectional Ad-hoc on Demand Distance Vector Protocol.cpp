...
{
  bool verbose = true;
  uint32_t nCsma = 25; //number of servers
  uint32_t nWifi = 17; //number of clients
  bool tracing = true;
...
//float xn[21] = {-27.4,-217.5,-515.1,-841.70,-36.7,-386.6,-442.3,-603.3,-726.5,-863.4,-130.9,-336.3,-600.2,-890.2,-986.2,-255.5,-533.6,-613.2,-697.7,-747.7,-917.2};

float dy [4]={2,6,10,14};

  std::cout << "Creating " << 21 << " nodes in 4 lanes and " << 4 << " m apart.\n";
//  nodes.Create (21);
  // Name nodes
  /*for (uint32_t i = 0; i < (uint32_t)10; ++i)
	{
...
mobility.SetPositionAllocator ("ns3::GridPositionAllocator",
                             	"MinX", DoubleValue (0.0),
                             	"MinY", DoubleValue (0.0),
                             	"DeltaX", DoubleValue (u*(x[i]-x[i-1])),
                             	"DeltaY", DoubleValue (dy[j]),
                             	"GridWidth", UintegerValue (10),
                             	"LayoutType", StringValue ("RowFirst"));
   mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
...
y->SetAttribute ("Mean", DoubleValue (mean));
...
std::cout<<val;
                      	...
                      	echoClient[j].SetAttribute ("PacketSize", UintegerValue (val));
...
// Run simulation
  ...
  Simulator::Destroy ();
  for(int k=0;k<10;k++){
      	Ptr<Node> node = wifiStaNodes.Get (k);
      	Ptr<MobilityModel> mob = node->GetObject<MobilityModel> ();
      	Simulator::Schedule (Seconds (100/3), &MobilityModel::SetPosition, mob, Vector (1e5, 1e5, 1e5));

}
  return 0;
}

-file-style:"gnu"; indent-tabs-mode:nil; -*- */
/*
 * This program is free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License version 2 as
 * published by the Free Software Foundation;
